...
describe('Testing in fileUpload', () => {
  test('should upload files correctly on cloudinary ', async () => {
    ...
    const blob = await resp.blob(); //El metodo blob() se usa para extraer el archivo propiamente dicho (imagen, video, etc) y guardarlo en la constante blob, se lo podria ver tambien como descargar la imagen con .blob() y guardarla en blob
